...
function createLaplacian(nX, nY, h)
    ...
    for x=1:nX
        for y=1:nY
            ...
            for i=1:length(dx)
                xNeighbour = (nX+x+dx[i]-1)%(nX)+1 #arrayLoop(x+dx[i],nX) # Find (x,y) indices of neighbouring grid point, introducing periodicity with arrayLoop
                yNeighbour = (nY+y+dy[i]-1)%(nY)+1 #arrayLoop(y+dy[i],nY) # Find (x,y) indices of neighbouring grid point, introducing periodicity with arrayLoop
