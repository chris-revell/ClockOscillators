...
rod   = Observable([Point2f0(0, 0), Point2f0(x1, y1), Point2f0(x2, y2)])
balls = Observable([Point2f0(x1, y1), Point2f0(x2, y2)])
# (Remember: the most optimal way to plot 2D things in Makie.jl is to
# give it a vector of `Point2f0`, the coordinates for the plot)
...
fill!(traj, Point2f0(x2, y2)) # add correct values to the circular buffer
...
function animstep!(integ, rod, balls, traj)
    ...
    rod[] = [Point2f0(0, 0), Point2f0(x1, y1), Point2f0(x2, y2)]
    balls[] = [Point2f0(x1, y1), Point2f0(x2, y2)]
    push!(traj[], Point2f0(x2, y2))
...
on(spoint) do z
    ...
    traj[] .= fill(Point2f0(x2, y2), length(traj[]))
    traj[] = traj[]
    rod[] = [Point2f0(0, 0), Point2f0(x1, y1), Point2f0(x2, y2)]
    balls[] = [Point2f0(x1, y1), Point2f0(x2, y2)]
end
